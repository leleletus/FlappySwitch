...
// Incluye aquí la tabla ASCII (5x8) que ya tienes
              ...
              int scale, uint32_t color)
{
  ...
  const uint8_t *glyph = ASCII[(uint8_t)c - 0x20];
  for (int col = 0; col < 5; col++) {
    uint8_t bits = glyph[col];
    for (int row = 0; row < 8; row++) {
      if (bits & (1 << row)) {
        // dibuja bloque scale×scale
        for (int dy = 0; dy < scale; dy++) {
          for (int dx = 0; dx < scale; dx++) {
            uint32_t *pixel =
              (uint32_t*)(fbptr
                + (y + row*scale + dy) * stride
                + (x + col*scale + dx) * 4);
            *pixel = color;
          }
        }
                ...
                int scale, uint32_t color)
{
  int cx = x;
  while (*s) {
    drawChar(fbptr, stride, cx, y, *s++, scale, color);
    cx += (5 + 1) * scale;  // 5 px + 1 px espacio
  }
}
    ...
    for (int page = 0; page < pages; page++) {
        for (int bx = 0; bx < w; bx++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                ...
                int py_base = y + (page * 8 + bit) * scale;
...
void init_animation(Animation* anim,
    ...
    anim->tick_per_frame  = tick_per_frame;
    anim->width           = w;
    anim->height          = h;
    anim->scale           = scale;
    anim->tick            = 0;
...
void update_animation(Animation* anim) {
    if (++anim->tick >= anim->tick_per_frame) {
        anim->tick = 0;
        anim->frame_index =
            (anim->frame_index + 1) % anim->frame_count;
    }
}
